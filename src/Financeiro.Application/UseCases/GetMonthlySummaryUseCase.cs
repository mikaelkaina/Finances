...
public class GetMonthlySummaryUseCase
{
    ...
    public async Task<MonthlySummaryOutput> ExecuteAsync(
        string userId,
        int month,
        int year)
    {
        var totalIncome =
            await _incomeRepository.GetTotalByMonthAsync(userId, month, year);

        var totalExpense =
            await _expenseRepository.GetTotalByMonthAsync(userId, month, year);
